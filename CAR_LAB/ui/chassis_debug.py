...
class ChassisDebugPage(QWidget):
    def __init__(self,bus,transport,config):
        ...
        mb=QGroupBox("多电机 RPM 一致性"); mg=QGridLayout(mb); self.pwm=QDoubleSpinBox();self.pwm.setRange(0,50);self.pwm.setValue(15);self.duration=QDoubleSpinBox();self.duration.setRange(.5,10);self.duration.setValue(2.0);run=QPushButton("开始一致性测试");run.setObjectName("primary");run.clicked.connect(self._start_consistency);stop=QPushButton("全部停止");stop.setObjectName("danger");stop.clicked.connect(lambda:self.transport.command("emergency_stop",True));mg.addWidget(QLabel("PWM %"),0,0);mg.addWidget(self.pwm,0,1);mg.addWidget(QLabel("时间(s)"),0,2);mg.addWidget(self.duration,0,3);mg.addWidget(run,1,0,1,2);mg.addWidget(stop,1,2,1,2);root.addWidget(mb)
