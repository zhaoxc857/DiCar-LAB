...
- 精准控制台壳层：顶部导航固定提供“概览、实时调试、波形记录、诊断”四个真实入口，连接、硬件指南和偏好集中在设备抽屉。
...
0.2.0 在首个硬件兼容版本上增加安全桌面 AI 通道和本机原始波形记录/回放；当前源码随后完成了精准控制台 UI 优化。现有 `release/` 二进制没有在本轮纯前端阶段重新构建，后续发布前须重新执行完整门禁与打包。项目仍处于早期迭代阶段，真实车辆接入前仍应先在断电、安全架起或低功率条件下验证参数范围与控制方向。
