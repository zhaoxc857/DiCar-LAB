...
- **单文件版**（推荐）：下载 `DiCAR-LAB-v1.12.0-Windows-x64-onefile.exe`，它是一个独立的 exe，放到任意位置双击即用，无需解压。启动时程序会先自解压，首次窗口弹出约需 2~3 秒，属正常现象。
- **便携文件夹版**：下载 `DiCAR-LAB-v1.12.0-Windows-x64.zip`，完整解压（不要直接在压缩包内运行）后双击其中的 `DiCAR LAB.exe`。
...
```powershell
Get-FileHash .\DiCAR-LAB-v1.12.0-Windows-x64-onefile.exe -Algorithm SHA256
```
...
# 生成 release\DiCAR-LAB-v1.12.0-Windows-x64.zip
.\build_portable_windows.bat

# 生成 release\DiCAR-LAB-v1.12.0-Windows-x64-onefile.exe（单文件版）
