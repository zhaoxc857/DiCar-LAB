...
export function ChangeBar({ dirtyCount, onReview }: { dirtyCount: number; onReview: () => void }) {
  const bridge = useDesktopBridge();
  const profile = useCollaborationStore((state) => state.profile);
  const [message, setMessage] = useState<string | null>(null);
  const commitReason = profile.role !== "owner" ? "当前身份没有固化权限" : !profile.leaseActive ? "当前车辆控制权未激活" : dirtyCount === 0 ? "没有待固化参数" : null;
  ...
  return <aside className="sticky bottom-3 z-20 mt-4 flex flex-wrap items-center justify-between gap-3 rounded-[var(--radius)] border border-(--border) bg-[color-mix(in_srgb,var(--surface-raised)_94%,transparent)] p-3 shadow-xl backdrop-blur">
    ...
    <div className="flex gap-2"><Button onClick={() => void run("undo")} size="sm" variant="secondary">撤销上次</Button><Button disabled={dirtyCount === 0} onClick={() => void run("revert")} size="sm" variant="secondary">全部回退</Button><Button disabled={commitReason !== null} onClick={onReview} size="sm">审阅并固化</Button></div>
  </aside>;
}
