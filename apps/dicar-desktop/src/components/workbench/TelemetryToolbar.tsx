...
export function TelemetryToolbar(props: TelemetryToolbarProps) {
  ...
  return <div className="mt-3 rounded-[var(--radius)] border border-(--border) bg-(--background) p-3">
    <div className="flex flex-wrap items-end gap-2">
      <label className="min-w-28 text-[10px] text-(--text-muted)">采样率<Select aria-label="遥测采样率" className="mt-1 h-8 text-xs" onChange={(event) => props.onSampleRate(Number(event.currentTarget.value))} value={props.sampleRateHz}>{[100, 200, 500].map((rate) => <option key={rate} value={rate}>{rate} Hz</option>)}</Select></label>
      ...
      <Button aria-expanded={channelsOpen} onClick={() => setChannelsOpen((open) => !open)} size="sm" variant="secondary">选择通道 {props.selectedIds.length}/8</Button>
    </div>
