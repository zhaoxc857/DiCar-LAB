...
export function TelemetryToolbar(props: TelemetryToolbarProps) {
  ...
  return <div className="mt-3 rounded-[var(--radius)] border border-(--border) bg-(--background) p-3">
    <div className="flex flex-wrap items-end gap-2">
      ...
      <Button
        aria-label={props.recordingActive ? "停止波形记录" : "开始波形记录"}
        onClick={props.recordingActive ? props.onStopRecording : props.onStartRecording}
        size="sm"
        variant={props.recordingActive ? "danger" : "secondary"}
      >
        {props.recordingActive ? "停止记录" : "开始记录"}
      </Button>
    ...
    {props.recordingActive && <p aria-live="polite" className="m-0 mt-2 text-[10px] text-(--danger)">正在记录 · {props.recordingName}</p>}
