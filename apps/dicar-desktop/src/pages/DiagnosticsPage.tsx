  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-6 lg:px-6" id="main-content">
      ...
      <section className="mt-5 grid gap-3 lg:grid-cols-3" aria-label="设备身份">
        ...
        <Identity label="设备 ID" value={snapshot?.deviceIdHex ?? "—"} /><Identity label="遥测安全上限" value={budget ? `${budget.maxChannels} 通道 × ${budget.maxSampleRateHz} Hz` : "—"} /><Identity label="断开原因" value={snapshot?.lastDisconnectReason ?? "无"} />
      </section>
      {budget && <p className="mt-3 rounded border border-(--border) bg-(--surface-raised) px-3 py-2 text-xs text-(--text-muted)">{budget.reason}</p>}
